...
const DEMO=[["Cricket","Malaysia","United Arab Emirates"],["Football","North City FC","United Stars"],["Tennis","Player A","Player B"]];
const fmt=n=>Number(n||0).toLocaleString("en-IN");
function App(){const[page,setPage]=useState("home"),[sport,setSport]=useState("cricket"),[events,setEvents]=useState([]),[match,setMatch]=useState(null),[slip,setSlip]=useState([]),[favorites,setFavorites]=useState([]),[token,setToken]=useState(localStorage.getItem("nova")||""),[user,setUser]=useState(null),[auth,setAuth]=useState(false),[search,setSearch]=useState(""),[toast,setToast]=useState("");
useEffect(()=>{if(token)fetch("/api/me",{headers:{Authorization:"Bearer "+token}}).then(r=>r.ok?r.json():null).then(x=>x&&setUser(x.user))},[token]);
useEffect(()=>{load();const id=setInterval(load,30000);return()=>clearInterval(id)},[sport]);
async function load(){
  let out=[];
  try{
    const ss=await fetch("/api/sports").then(r=>r.ok?r.json():[]);
    const wanted=sport==="soccer"?"soccer":sport;
    const keys=ss.filter(x=>x.active&&((x.group||"").toLowerCase()===wanted||(x.key||"").toLowerCase().startsWith(wanted+"_")))
      .map(x=>x.key).filter(Boolean).slice(0,8);
    for(const key of keys){
      const r=await fetch("/api/odds/"+encodeURIComponent(key)+"?regions=eu&markets=h2h,h2h_lay,spreads,totals");
      if(r.ok){const d=await r.json();if(Array.isArray(d))out.push(...d.map(x=>({...x,sport_key:key})))}
    }
  }catch(err){console.log(err)}
  out=[...new Map(out.map(x=>[x.id,x])).values()].sort((a,b)=>new Date(a.commence_time)-new Date(b.commence_time));
  if(!out.length)out=DEMO.filter(x=>x[0].toLowerCase()===(sport==="soccer"?"football":sport)).map((x,i)=>({id:"d"+i,home_team:x[1],away_team:x[2],sport_title:x[0],commence_time:new Date(Date.now()+i*3600000).toISOString(),demo:true,bookmakers:[{markets:[{key:"h2h",outcomes:[{name:x[1],price:i===0?1.72:1.84},{name:x[2],price:i===0?2.12:2.04}]}]}]}));
  setEvents(out)
}
function add(e,o,m,side="back",stake=100){if(!token){setAuth(true);return}let x={eventId:e.id,eventName:(e.home_team||"")+" vs "+(e.away_team||""),selection:o.name,market:m.key,side,odds:+o.price,stake:Number(stake)||100};setSlip(a=>[...a.filter(z=>z.eventId!==x.eventId||z.selection!==x.selection||z.side!==x.side),x]);setToast((side==="lay"?"Lay":"Back")+" selection added to virtual slip")}
function layPrice(backOdds){const o=Number(backOdds)||0;return o>5?o+1:o+0.04}
function calc(item){const s=Number(item.stake)||0,o=Number(item.odds)||0;return item.side==="lay"?{win:s,loss:s*Math.max(0,o-1)}:{win:s*Math.max(0,o-1),loss:s}}
async function coins(){if(!token)return setAuth(true);let r=await fetch("/api/demo-coins",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({amount:1000})});let d=await r.json();if(r.ok){setUser(d.user);setToast("+1,000 virtual coins")}}
...
<main>{page==="home"&&<Home setPage={setPage} sport={sport} setSport={setSport} events={events} open={e=>{setMatch(e);setPage("match")}} add={add} coins={coins} search={search} setSearch={setSearch} favorites={favorites} setFavorites={setFavorites}/>}
{page==="sports"&&<Sports sport={sport}setSport={setSport}events={events}open={e=>{setMatch(e);setPage("match")}}add={add} search={search} setSearch={setSearch} favorites={favorites} setFavorites={setFavorites}/>}
{page==="match"&&<Match e={match}back={()=>setPage("sports")}add={add}setMatch={setMatch}/>}
{page==="slip"&&<Slip items={slip}setSlip={setSlip}place={async(x)=>{let r=await fetch("/api/bets",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+token},body:JSON.stringify({...x,stake:100})});let d=await r.json();if(r.ok){setUser(d.user);setSlip(a=>a.filter(z=>z!==x));setToast("Virtual bet placed")}else setToast(d.error)}}/>}
...
{page==="history"&&<History token={token}/>} {page==="games"&&<Games/>}</main>
<nav><button onClick={()=>setPage("home")}>⌂<small>Home</small></button><button onClick={()=>setPage("sports")}>◈<small>Sports</small></button><button className="center"onClick={()=>setPage("slip")}>▱<i>{slip.length}</i></button><button onClick={()=>setPage("games")}>◇<small>Games</small></button><button onClick={()=>user?setPage("profile"):setAuth(true)}>♙<small>Account</small></button></nav>
...
function Home(p){return <div className="page"><section className="hero"><div><div className="livePill"><i/> LIVE FEED CONNECTED</div><small>WELCOME TO NOVA PLAY</small><h1>Next-level<br/><em>play zone.</em></h1><p>Live sports, smart markets and a premium virtual-coin experience.</p><div className="heroActions"><button className="primary"onClick={()=>p.setPage("sports")}>Explore Sports →</button><button className="ghost"onClick={()=>p.setPage("games")}>Game Lounge</button></div></div><div className="orb">🪙<small>VIRTUAL<br/>ONLY</small></div></section><div className="stats"><div><b>LIVE</b><small>Sports feed</small></div><div><b>4+</b><small>Sports</small></div><div><b>24/7</b><small>Demo access</small></div></div><div className="quick"><button onClick={()=>p.setPage("sports")}>⚡<b>Live Sports</b><small>Real-time feed</small></button><button onClick={()=>p.setPage("games")}>🎮<b>Game Lounge</b><small>Demo games</small></button><button onClick={p.coins}>🎁<b>Daily Coins</b><small>+1,000 demo</small></button></div><div className="searchBox"><span>⌕</span><input id="eventSearch"placeholder="Search teams, players or leagues"value={p.search}onChange={e=>p.setSearch(e.target.value)}/></div><h2>Popular sports</h2><Rail {...p}/><div className="sectionLine"><h2>Live & upcoming</h2><button onClick={()=>p.setPage("sports")}>View all →</button></div><Events {...p}/><div className="note">🛡️ Virtual coins only · No cash value</div></div>}
