...
const DEMO=[
 ["Cricket","Malaysia","United Arab Emirates"],
 ["Football","North City FC","United Stars"],
 ["Tennis","Player A","Player B"],
 ["Basketball","City Hoops","United Hoops"]
];
const fmt=n=>Number(n||0).toLocaleString("en-IN");
const isLive=e=>!e?.completed&&new Date(e?.commence_time||0).getTime()<=Date.now();
const layPrice=back=>{const o=Number(back)||0;return o>5?Number((o+1).toFixed(2)):Number((o+0.04).toFixed(2));};
...
function App(){
 ...
 async function load(){
  ...
  try{
   ...
   const wanted=sport;
   const keys=ss.filter(x=>x.active&&((x.group||"").toLowerCase()===wanted||(x.key||"").toLowerCase().startsWith(wanted+"_"))).map(x=>x.key).filter(Boolean).slice(0,8);
  ...
  if(!out.length){
   const wanted=sport==="soccer"?"Football":sport[0].toUpperCase()+sport.slice(1);
   out=DEMO.filter(x=>x[0]===wanted).map((x,i)=>({id:"demo-"+sport+"-"+i,home_team:x[1],away_team:x[2],sport_title:x[0],commence_time:new Date(Date.now()+(i?i*3600000: -1800000)).toISOString(),completed:false,demo:true,bookmakers:[{title:"NOVA Virtual Exchange",markets:[{key:"h2h",outcomes:[{name:x[1],price:i===0?1.72:1.84},{name:x[2],price:i===0?2.12:2.04}]}]}]}));
  }
  setEvents(out);
 }
...
function Home({setPage,sport,setSport,events,open,search,setSearch,favorites,setFavorites,filter,setFilter,chooseBet}){
 ...
 return <div className="page homePage">
  ...
  <div className="homeFilters"><button className={filter==="all"?"active":""}onClick={()=>setFilter("all")}>All</button><button className={filter==="live"?"active":""}onClick={()=>setFilter("live")}>● Live</button><button className={filter==="upcoming"?"active":""}onClick={()=>setFilter("upcoming")}>Upcoming</button></div>
...
function EventCard({e,open,favorites,setFavorites,chooseBet}){
 const m=e.bookmakers?.flatMap(b=>b.markets||[]).find(x=>x.key==="h2h")||e.bookmakers?.[0]?.markets?.find(x=>x.key==="h2h");
 const os=m?.outcomes||[];const live=isLive(e);const fav=favorites.includes(e.id);
